...
class Member:
    def __init__(self, email, name, surname, address, postalCode, city, phone):
        ...
        self.lastTypePayment = None #or use self.receipts
        self.notes = None
    ...
    def toArray(self):
        return [self.email, self.name, self.surname, ";".join([receipt.id for receipt in self.receipts]), self.status, self.lastMembership, self.sendingMail, self.address, self.postalCode, self.city, self.phone, 0, 0, 0, 0, 0, self.lastPayment.strftime("%d/%m/%Y"), self.lastTypePayment, self.notes]